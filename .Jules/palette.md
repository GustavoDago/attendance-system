...
## 2025-05-17 - Enhancing Admin User Management UX and Accessibility
**Learning:** Redundant ARIA labels on nested elements (e.g., emojis inside already labeled buttons) can cause garbled or repetitive announcements for screen reader users. Additionally, asynchronous state updates in lists are safer when using functional updates and robust unmount guards.
**Action:** Always mark decorative emojis inside labeled buttons with `aria-hidden="true"`. Use functional state updates (`prev => ...`) for list modifications and implement an `isMounted` check that guards state updates within async functions.
